...
df_CDB = df_CDB.groupby(['block_id', 'Date']).agg({
    'Stop': ['first', 'last'],
...
df_HEB = df_HEB.groupby(['block_id', 'Date']).agg({
    'Stop': ['first', 'last'],
    'ServiceDateTime': ['min', 'max'],
    'Route': 'first',
    'Energy':'sum',
...
df_BEB = df_BEB.groupby(['block_id', 'Date']).agg({
    'Stop': ['first', 'last'],
    'ServiceDateTime': ['min', 'max'],
    'Route': 'first',
    'Energy':'sum',
...
model.setParam('Threads', 6)  # Set number of threads to be used for parallel processing.
