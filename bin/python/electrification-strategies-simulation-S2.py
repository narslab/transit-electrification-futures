...
# Define parameters
bus_costs = {'BEB':1300000, 'HEB':600000, 'CDB':0}
...
problem += x[13]['HEB'] == 198  # convert all CDB to HEB by the 13th year
problem += x[13]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[13]['CDB'] == 0    # total CDB remains at 9 over all years
problem += x[12]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[11]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[10]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[9]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[8]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[7]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[6]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[5]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[4]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[3]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[2]['BEB'] == 15   # total BEB remains at 15 over all years
problem += x[1]['BEB'] == 15   # total BEB remains at 15 over all years
...
purchase_plan = pd.DataFrame({
    ...
    'VehicleModel': ['NEW FLYER XDE40', 'NEW FLYER XDE40', 'NEW FLYER XDE40'],
    'VehiclWeight(lb)': [28250, 28250, 28250]
})
...
vehicles_to_replace.to_csv(r'../../results/S2-vehicles_to_replace.csv')
...
for year in vehicles_to_replace['Year'].unique():
    ...
    output_csv_filename = f'computed-fuel-rates-S2-{year}.csv'
