...
def process_dataframe(df, validation, gamma):
    ...
    df_integrated=df_integrated.loc[df_integrated['trip']!=0]
...
def calibrate_parameter(args):
    ...
    gamma_values = np.around(np.linspace(start_1, stop_1, n_points_1), decimals=decimal_places)
    driveline_efficiencies = np.around(np.linspace(start_2, stop_2, n_points_2))
    ...
    for gamma in tqdm(gamma_values, desc="Processing gamma values"):
        for driveline_efficiency in tqdm(driveline_efficiencies, desc="Processing driveline_efficiencies values"):
            ...
            df_integrated = df_integrated.loc[df_integrated['Energy_sum']!=0]
            df_integrated = df_integrated.groupby('Transaction Date')[['dist','trip', 'Energy_sum']].sum()        
            ...
            parameter1_values.append(driveline_efficiency)
...
calibrate_parameter((0.0001,1, 10, 0,9, 0,99,5))